...
function collectTransactionData($transactionRequest, $client)
{
    ...
    foreach (SdkRestApi::getParam('basketItems') as $key => $basketItem) {
        $prefixIfDuplicated = TrustPaymentsSdkHelper::checkForDuplicatePrefix($basketItem['plenty_basket_row_item_variation_id'], $arrayOfItemIdsInLoop, $key);
        ...
        if ($basketNetPrices && isset($basketItem['vat']) && ! empty($basketItem['vat'])) {
            $lineItem->setAmountIncludingTax(TrustPaymentsSdkHelper::roundAmount(($basketItem['price'] / (1 + $basketItem['vat'] / 100)) * $basketItem['quantity'], $currencyDecimalPlaces));
        } else {
            $lineItem->setAmountIncludingTax(TrustPaymentsSdkHelper::roundAmount($basketItem['price'] * $basketItem['quantity'], $currencyDecimalPlaces));
        }
    ...
    if ($basket['shippingAmount'] > 0) {
        ...
        $lineItem->setAmountIncludingTax(TrustPaymentsSdkHelper::roundAmount($basket['shippingAmount'], $currencyDecimalPlaces));
    ...
    if ($basket['couponDiscount'] < 0) {
        ...
        $lineItem->setQuantity(1);
        $lineItem->setAmountIncludingTax(TrustPaymentsSdkHelper::roundAmount($basket['couponDiscount'], $currencyDecimalPlaces));
    ...
    if ($basket['paymentAmount'] > 0) {
        ...
        $lineItem->setQuantity(1);
        $lineItem->setAmountIncludingTax(TrustPaymentsSdkHelper::roundAmount($basket['paymentAmount'], $currencyDecimalPlaces));
        $lineItem->setType('FEE');
        $lineItems[] = $lineItem;
    }
    $lineItemTotalAmount = TrustPaymentsSdkHelper::calculateLineItemTotalAmount($lineItems);
    $basketAmount = $basketForTemplate['basketAmount'];
    if (TrustPaymentsSdkHelper::roundAmount($lineItemTotalAmount, $currencyDecimalPlaces) > TrustPaymentsSdkHelper::roundAmount($basketAmount, $currencyDecimalPlaces)) {
        $lineItem = new LineItemCreate();
        $lineItem->setUniqueId('adjustment');
        $lineItem->setSku('adjustment');
        $lineItem->setName('Adjustment');
        $lineItem->setQuantity(1);
        $lineItem->setAmountIncludingTax(TrustPaymentsSdkHelper::roundAmount($basketAmount - $lineItemTotalAmount, $currencyDecimalPlaces));
        $lineItem->setType('DISCOUNT');
        $lineItems[] = $lineItem;
    ...
    $filter->setChildren([
        ...
        TrustPaymentsSdkHelper::createEntityFilter('paymentMethod', $paymentMethodId)
    ]);
...
if (! empty($transactionId)) {
    ...
    $transactionRequest->setCustomersPresence(\TrustPayments\Sdk\Model\CustomersPresence::VIRTUAL_PRESENT);
...
return TrustPaymentsSdkHelper::convertData($transactionResponse);
