...
use TrustPayments\Sdk\Model\LineItemAttributeCreate;
...
function buildLineItem($orderItem, $uniqueId, $sku, $type, $basketNetPrices, $currencyDecimalPlaces, $itemAttributes, $prefixIfDuplicated)
{
    ...
    if ($basketNetPrices) {
        $lineItem->setAmountIncludingTax(TrustPaymentsSdkHelper::roundAmount($itemAmount['priceNet'] * $orderItem['quantity'], $currencyDecimalPlaces));
    } else {
        $lineItem->setAmountIncludingTax(TrustPaymentsSdkHelper::roundAmount($itemAmount['priceGross'] * $orderItem['quantity'], $currencyDecimalPlaces));
    }
...
function collectTransactionData($transactionRequest, $client)
{
    ...
    foreach ($order['orderItems'] as $key => $orderItem) {
        $prefixIfDuplicated = TrustPaymentsSdkHelper::checkForDuplicatePrefix($orderItem['itemVariationId'], $arrayOfItemIdsInLoop, $key);
