...
class PaymentHelper
{
    ...
    /**
     * Returns the Trust Payments payment method's id.
     *
    ...
    {
        $paymentMethods = $this->paymentMethodRepository->allForPlugin('trustpayments');
     ...
     * @return string
     */
    public function isTrustPaymentsPaymentMopId($mopId): bool
    {
        $paymentMethods = $this->paymentMethodRepository->allForPlugin('trustpayments');
        if (! is_null($paymentMethods)) {
            foreach ($paymentMethods as $paymentMethod) {
