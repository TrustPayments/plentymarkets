...
use TrustPayments\Helper\OrderHelper;
...
class PaymentService
{
    ...
    public function getPaymentContent(Basket $basket, array $basketForTemplate, PaymentMethod $paymentMethod): array
    {
        ...
        $parameters = [
            'transactionId' => $this->session->getPlugin()->getValue('trustPaymentsTransactionId'),
            'basket' => $basket,
        ...
        $this->getLogger(__METHOD__)->debug('trustPayments::TransactionParameters', $parameters);

        $transaction = $this->sdkService->call('createTransactionFromBasket', $parameters);
        if (is_array($transaction) && isset($transaction['error'])) {
            $this->getLogger(__METHOD__)->error('trustPayments::TransactionError', $transaction);
            return [
        ...
        $this->session->getPlugin()->setValue('trustPaymentsTransactionId', $transaction['id']);
    ...
    private function createWebhook()
    {
        ...
        $this->sdkService->call('createWebhook', [
            ...
            'notificationUrl' => $webstoreConfig->domainSsl . '/trustpayments/update-transaction' . ($this->config->get('plenty.system.info.urlTrailingSlash', 0) == 2 ? '/' : '')
        ]);
    }
    ...
    public function executePayment(Order $order, PaymentMethod $paymentMethod): array
    {
        ...
        $this->session->getPlugin()->unsetKey('trustPaymentsTransactionId');
        ...
        if (is_array($existingTransaction) && $existingTransaction['error']) {
            $this->getLogger(__METHOD__)->error('trustPayments::ExistingTransactionsError', $existingTransaction);
            return [
                'transactionId' => $transactionId,
                'type' => GetPaymentMethodContent::RETURN_TYPE_ERROR,
        ...
        if (is_array($paymentPageUrl) && isset($paymentPageUrl['error'])) {
            $this->getLogger(__METHOD__)->error('trustPayments::PaymentPageUrlError', $paymentPageUrl);
            return [
                'transactionId' => $transaction['id'],
                'type' => GetPaymentMethodContent::RETURN_TYPE_ERROR,
        ...
        foreach ($order->orderItems as $orderItem) {
            if (! empty($orderItem->orderProperties)) {
                ...
                foreach ($orderItem->orderProperties as $orderProperty) {
                    ...
                    $this->getLogger(__METHOD__)->debug('trustPayments::Variation', [
    ...
    {
        return $this->webstoreHelper->getCurrentWebstoreConfiguration()->domainSsl . '/trustpayments/fail-transaction';
    }
    ...
    {
        $this->getLogger(__METHOD__)->debug('trustPayments:RefundOrder', [
            'transactionId' => $transactionId,
    ...
    {
        $status = $this->config->get('trustPayments.refund_successful_status');
        if (empty($status)) {
            return '11.2';
        } else {
            return $status;
    ...
    {
        $status = $this->config->get('trustPayments.refund_failed_status');
        if (empty($status)) {
            return '11.3';
        } else {
            return $status;
