...
class PaymentProcessController extends Controller
{
    ...
    public function failTransaction(Twig $twig, int $id)
    {
        ...
        $errorMessage = $this->frontendSession->getPlugin()->getValue('trustPaymentsPayErrorMessage');
        if ($errorMessage) {
            $this->notificationService->error($errorMessage);
            $this->frontendSession->getPlugin()->unsetKey('trustPaymentsPayErrorMessage');
        ...
        return $twig->render('trustPayments::Failure', [
    ...
    public function payOrder(Request $request)
    {
        ...
        if ($result['type'] == GetPaymentMethodContent::RETURN_TYPE_REDIRECT_URL) {
            ...
            if (isset($result['content'])) {
                $this->frontendSession->getPlugin()->setValue('trustPaymentsPayErrorMessage', $result['content']);
            }
            return $this->response->redirectTo('trustpayments/fail-transaction/' . $result['transactionId']);
        } else {
            return $this->response->redirectTo('confirmation');
        }
    }
    ...
    private function getPaymentMethodListForSwitch($paymentMethodId, $orderId)
    {
        ...
        foreach ($paymentMethods as $paymentMethod) {
            if ($paymentMethod->pluginKey == 'trustPayments') {
                $paymentMethodsForSwitch[] = [
                    'id' => $paymentMethod->id,
    ...
    private function checkOrderRetryStatus($statusId) {
        $orderRetryStatusString = $this->config->get('trustPayments.order_retry_status');
