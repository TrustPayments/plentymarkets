...
class EpsPaymentMethod extends AbstractPaymentMethod
{
    ...
    {
        if ($this->configRepo->get('trustPayments.eps_active') === "true") {
            return true;
        } else {
            return false;
        }
    ...
    {
        $title = $this->configRepo->get('trustPayments.eps_title');
        if (! empty($title)) {
            return $title;
        } else {
            return 'EPS';
    ...
    {
        $fee = $this->configRepo->get('trustPayments.eps_fee');
        if (! empty($fee)) {
            return (float) $fee;
        } else {
            return 0.00;
    ...
    {
        $title = $this->configRepo->get('trustPayments.eps_description');
        if (! empty($title)) {
            return $title;
        } else {
            return '';
    ...
    {
        $iconUrl = $this->configRepo->get('trustPayments.eps_icon_url');
        if (!empty($iconUrl)) {
            return $iconUrl;
        } else {
