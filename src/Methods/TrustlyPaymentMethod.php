...
class TrustlyPaymentMethod extends AbstractPaymentMethod
{
    ...
    {
        if ($this->configRepo->get('trustPayments.trustly_active') === "true") {
            return true;
        } else {
            return false;
        }
    ...
    {
        $title = $this->configRepo->get('trustPayments.trustly_title');
        if (! empty($title)) {
            return $title;
        } else {
            return 'Trustly';
    ...
    {
        $fee = $this->configRepo->get('trustPayments.trustly_fee');
        if (! empty($fee)) {
            return (float) $fee;
        } else {
            return 0.00;
    ...
    {
        $title = $this->configRepo->get('trustPayments.trustly_description');
        if (! empty($title)) {
            return $title;
        } else {
            return '';
    ...
    {
        $iconUrl = $this->configRepo->get('trustPayments.trustly_icon_url');
        if (!empty($iconUrl)) {
            return $iconUrl;
        } else {
