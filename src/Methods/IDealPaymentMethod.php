...
class IDealPaymentMethod extends AbstractPaymentMethod
{
    ...
    {
        if ($this->configRepo->get('trustPayments.ideal_active') === "true") {
            return true;
        } else {
            return false;
        }
    ...
    {
        $title = $this->configRepo->get('trustPayments.ideal_title');
        if (! empty($title)) {
            return $title;
        } else {
            return 'iDeal';
    ...
    {
        $fee = $this->configRepo->get('trustPayments.ideal_fee');
        if (! empty($fee)) {
            return (float) $fee;
        } else {
            return 0.00;
    ...
    {
        $title = $this->configRepo->get('trustPayments.ideal_description');
        if (! empty($title)) {
            return $title;
        } else {
            return '';
    ...
    {
        $iconUrl = $this->configRepo->get('trustPayments.ideal_icon_url');
        if (!empty($iconUrl)) {
            return $iconUrl;
        } else {
