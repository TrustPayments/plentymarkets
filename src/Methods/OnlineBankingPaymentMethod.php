...
class OnlineBankingPaymentMethod extends AbstractPaymentMethod
{
    ...
    {
        if ($this->configRepo->get('trustPayments.onlinebanking_active') === "true") {
            return true;
        } else {
            return false;
        }
    ...
    {
        $title = $this->configRepo->get('trustPayments.onlinebanking_title');
        if (! empty($title)) {
            return $title;
        } else {
    ...
    {
        $fee = $this->configRepo->get('trustPayments.onlinebanking_fee');
        if (! empty($fee)) {
            return (float) $fee;
        } else {
            return 0.00;
    ...
    {
        $title = $this->configRepo->get('trustPayments.onlinebanking_description');
        if (! empty($title)) {
            return $title;
        } else {
            return '';
    ...
    {
        $iconUrl = $this->configRepo->get('trustPayments.onlinebanking_icon_url');
        if (!empty($iconUrl)) {
            return $iconUrl;
        } else {
