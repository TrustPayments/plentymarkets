...
class InvoicePaymentMethod extends AbstractPaymentMethod
{
    ...
    {
        if ($this->configRepo->get('trustPayments.invoice_active') === "true") {
            return true;
        } else {
            return false;
        }
    ...
    {
        $title = $this->configRepo->get('trustPayments.invoice_title');
        if (! empty($title)) {
            return $title;
        } else {
            return 'Invoice';
    ...
    {
        $fee = $this->configRepo->get('trustPayments.invoice_fee');
        if (! empty($fee)) {
            return (float) $fee;
        } else {
            return 0.00;
    ...
    {
        $title = $this->configRepo->get('trustPayments.invoice_description');
        if (! empty($title)) {
            return $title;
        } else {
            return '';
    ...
    {
        $iconUrl = $this->configRepo->get('trustPayments.invoice_icon_url');
        if (!empty($iconUrl)) {
            return $iconUrl;
        } else {
