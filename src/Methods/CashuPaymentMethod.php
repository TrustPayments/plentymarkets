...
class CashuPaymentMethod extends AbstractPaymentMethod
{
    ...
    {
        if ($this->configRepo->get('trustPayments.cashu_active') === "true") {
            return true;
        } else {
            return false;
        }
    ...
    {
        $title = $this->configRepo->get('trustPayments.cashu_title');
        if (! empty($title)) {
            return $title;
        } else {
            return 'CASHU';
    ...
    {
        $fee = $this->configRepo->get('trustPayments.cashu_fee');
        if (! empty($fee)) {
            return (float) $fee;
        } else {
            return 0.00;
    ...
    {
        $title = $this->configRepo->get('trustPayments.cashu_description');
        if (! empty($title)) {
            return $title;
        } else {
            return '';
    ...
    {
        $iconUrl = $this->configRepo->get('trustPayments.cashu_icon_url');
        if (!empty($iconUrl)) {
            return $iconUrl;
        } else {
