...
class PayboxPaymentMethod extends AbstractPaymentMethod
{
    ...
    {
        if ($this->configRepo->get('trustPayments.paybox_active') === "true") {
            return true;
        } else {
            return false;
        }
    ...
    {
        $title = $this->configRepo->get('trustPayments.paybox_title');
        if (! empty($title)) {
            return $title;
        } else {
            return 'paybox';
    ...
    {
        $fee = $this->configRepo->get('trustPayments.paybox_fee');
        if (! empty($fee)) {
            return (float) $fee;
        } else {
            return 0.00;
    ...
    {
        $title = $this->configRepo->get('trustPayments.paybox_description');
        if (! empty($title)) {
            return $title;
        } else {
            return '';
    ...
    {
        $iconUrl = $this->configRepo->get('trustPayments.paybox_icon_url');
        if (!empty($iconUrl)) {
            return $iconUrl;
        } else {
