...
class TenpayPaymentMethod extends AbstractPaymentMethod
{
    ...
    {
        if ($this->configRepo->get('trustPayments.tenpay_active') === "true") {
            return true;
        } else {
            return false;
        }
    ...
    {
        $title = $this->configRepo->get('trustPayments.tenpay_title');
        if (! empty($title)) {
            return $title;
        } else {
            return 'Tenpay';
    ...
    {
        $fee = $this->configRepo->get('trustPayments.tenpay_fee');
        if (! empty($fee)) {
            return (float) $fee;
        } else {
            return 0.00;
    ...
    {
        $title = $this->configRepo->get('trustPayments.tenpay_description');
        if (! empty($title)) {
            return $title;
        } else {
            return '';
    ...
    {
        $iconUrl = $this->configRepo->get('trustPayments.tenpay_icon_url');
        if (!empty($iconUrl)) {
            return $iconUrl;
        } else {
